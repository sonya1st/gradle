...
The Wrapper JAR is an executable binary file that is usually checked in to a project's version control system.
As such, there is the potential for a malicious actor to replace the original JAR with a modified one thereby injecting arbitrary code that will be executed on the computers of developers and build servers.
In order to allow checking the integrity of the Wrapper JAR, Gradle publishes the checksums of all releases (except for Gradle 3.3, 3.4, 3.4.1, 3.5, 3.5.1, 4.0, 4.0.1, and 4.0.2) alongside the corresponding Gradle distribution on https://services.gradle.org/.
You can verify the checksum of the Wrapper JAR to ensure that it has not been tampered with, for example when reviewing a pull request that upgrades the Wrapper JAR in an open source project.
Here are the commands you can run on the major operating systems to manually verify a Wrapper JAR's checksum:
...
> $actual = (Get-FileHash gradle-wrapper.jar -Algorithm SHA256).Hash.ToLower()
...
Once you know the actual checksum, check whether it's listed on https://gradle.org/wrapper-checksums/.
If so, you should run the `wrapper` task again to update the Wrapper JAR so it matches the Gradle version in `gradle/wrapper/gradle-wrapper.properties`.
If not, the Wrapper JAR might be from a milestone, release candidate, or nightly build -- or it might not be legit.
You should try to find out how it was generated but treat it as untrustworthy until proven otherwise.
If you think it was manipulated, please let the Gradle team know by sending an email to mailto:security@gradle.com[security@gradle.com].
